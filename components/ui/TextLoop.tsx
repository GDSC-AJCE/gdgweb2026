...
const buildPath = (shape: string, curviness: number, ribbonWidth: number, viewH: number, viewW: number = 1440) => {
  ...
  switch (shape) {
    ...
    case "arch": {
      ...
      return `M -200 ${CY + rise / 2} Q ${CX} ${CY - rise * 1.5} ${viewW + 200} ${CY + rise / 2}`;
    }
    case "line":
      return `M -500 ${CY} L ${viewW + 500} ${CY}`;
    case "wave":
    default: {
      const a = Math.min(c * 2.2, room * 2);
      const step = 320;
      let d = `M -640 ${CY}`;
      for (let x = -640; x <= viewW + 640; x += step * 2) {
        d += ` Q ${x + step / 2} ${CY - a} ${x + step} ${CY} T ${x + step * 2} ${CY}`;
      }
      return d;
    }
  }
...
const TextLoop: React.FC<TextLoopProps> = ({
  ...
  const d = useMemo(
    () => path || buildPath(shape, curviness, ribbonWidth, viewBoxHeight, VIEW_W),
  ...
  return (
    <div ref={rootRef} className={`text-loop ${className}`.trim()} style={style}>
      <svg
        ...
        viewBox={`0 0 ${VIEW_W} ${viewBoxHeight}`}
